...
class Media {
  constructor({
    ...
    Object.assign(this, {
      geometry, gl, image, index, length, renderer, scene,
      screen, text, viewport, bend, textColor, borderRadius, font,
    });
    this.createShader();
    this.createMesh();
    this.createTitle();
...
class App {
  ...
  createMedias(items, bend = 1, textColor, borderRadius, font) {
    const pastelColors = ['#f9d5e5','#e8d5f9','#d5f9ea','#fde8d5','#d5eef9','#fdf5d5'];
    const makeColor = (hex) =>
      `data:image/svg+xml,${encodeURIComponent(
        `<svg xmlns='http://www.w3.org/2000/svg' width='800' height='600'><rect width='800' height='600' fill='${hex}'/><text x='50%' y='50%' fill='rgba(45,41,53,0.25)' font-size='80' text-anchor='middle' dominant-baseline='middle' font-family='serif'>✦</text></svg>`
      )}`;

    const defaultItems = [
      { image: makeColor(pastelColors[0]), text: 'Generative AI · Udemy' },
      { image: makeColor(pastelColors[1]), text: 'Privacy & Security · NPTEL' },
      { image: makeColor(pastelColors[2]), text: 'Hardware & OS · Coursera' },
      { image: makeColor(pastelColors[3]), text: 'Computer Comms · Coursera' },
      { image: makeColor(pastelColors[4]), text: 'Networking · Google · Coursera' },
    ];
  ...
  addEventListeners() {
    ...
    window.addEventListener('mousedown',  this.boundTouchDown);
    window.addEventListener('mousemove',  this.boundTouchMove);
    window.addEventListener('mouseup',    this.boundTouchUp);
    window.addEventListener('touchstart', this.boundTouchDown);
  ...
  destroy() {
    ...
    window.removeEventListener('mousedown',  this.boundTouchDown);
    window.removeEventListener('mousemove',  this.boundTouchMove);
    window.removeEventListener('mouseup',    this.boundTouchUp);
    window.removeEventListener('touchstart', this.boundTouchDown);
...
export default function CircularGallery({
  ...
  return (
    <div
      ...
      tabIndex={0}
      role="region"
      aria-label="Certificate gallery. Use Left and Right Arrow keys to navigate."
    />
  );
}
